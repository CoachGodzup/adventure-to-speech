...
- [ ] `src/background.js` — minimal module service worker (`onInstalled` defaults, `ATS_STATE` forwarding)
- [ ] `src/popup.html` / `popup.js` / `popup.css` — enabled toggle, voice select, rate/pitch/volume sliders, autoRead + speakCommands checkboxes, Read-last + Stop buttons (400px max, `chrome.*`/`browser.*` fallback)
